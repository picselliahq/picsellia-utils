...
class COCOAnnotation(ABC):
    ...
    def add_image(self, img_filename: str, img_height: int, img_width: int):
        img = {"file_name": img_filename, "height": img_height, "width": img_width,
    ...
    def add_polygon_annotation(
        ...
        polygon_annotation = {"segmentation": polygon_list, "iscrowd": 0, "image_id": img_id, "bbox": [],
                              "category_id": category_id, "id": len(self.coco_annotations_dict["annotations"])}
        self.coco_annotations_dict["annotations"].append(polygon_annotation)

    def save_coco_annotations_as_json(self, json_path):
        save_dict_as_json_file(dictionary=self.coco_annotations_dict, json_path=json_path)
