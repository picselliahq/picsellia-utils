...
class AbstractConverter(ABC):
    ...
    def update_coco_annotations(self):
        ...
        for label_img_dir in labels_images_directories:
            ...
            label_mask_dir = find_most_similar_string(label_img_dir, labels_masks_directories)
            category_id = self._get_category_id_from_label_directory(label_directory=label_img_dir)
            img_filenames = os.listdir(os.path.join(self._images_dir, label_img_dir))

            for img_filename in img_filenames:
                img = imread(os.path.join(self._images_dir, label_img_dir, img_filename))
                ...
                masks_filepaths = self._get_masks_filepaths_of_image(img_filename=img_filename,
                                                                     label_dir=label_mask_dir)
                for mask_filepath in masks_filepaths:
                    formatted_polygons = self._get_formatted_polygons_from_mask(mask_filepath=mask_filepath,
